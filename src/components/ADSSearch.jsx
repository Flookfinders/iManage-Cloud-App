...
function ADSSearch({ variant, placeholder, onSearchClick }) {
  ...
  const searchLlpgStreets = useRef(null);
  ...
  async function handleSearchClick() {
    searchStreets.current = userContext.currentUser.hasStreet
      ? await Promise.all(
          data
            .filter((x) => x.type === 15)
            .map(async (x) => {
    ...
    searchLlpgStreets.current = !userContext.currentUser.hasStreet
      ? await Promise.all(
          data
            .filter((x) => x.type === 15)
            .map(async (x) => {
